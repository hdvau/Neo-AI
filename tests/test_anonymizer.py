...
class TestSeeding(unittest.TestCase):
    def setUp(self):
        self.a = PromptAnonymizer()
        self.a.seed(username="dha", hostname="bermudaserver")

    def test_username_replaced(self):
        result = self.a.anonymize("Logged in as dha")
        self.assertNotIn("dha", result)
        self.assertIn("[USER_1]", result)

    def test_hostname_replaced(self):
        result = self.a.anonymize("Host bermudaserver is up.")
        self.assertNotIn("bermudaserver", result)
        self.assertIn("[HOST_1]", result)

    def test_username_in_path_replaced(self):
        result = self.a.anonymize("Working dir: /home/dha/projects/neo")
        self.assertNotIn("dha", result)

    def test_username_not_replaced_as_substring(self):
        # "dha" inside "dashboard" should NOT be replaced
...
class TestDeanonymization(unittest.TestCase):
    def setUp(self):
        self.a = PromptAnonymizer()
        self.a.seed(username="dha", hostname="myserver")
    ...
    def test_roundtrip(self):
        original = "ssh dha@10.0.0.5 -i /home/dha/.ssh/id_rsa"
        anonymized = self.a.anonymize(original)
        self.assertNotIn("dha", anonymized)
